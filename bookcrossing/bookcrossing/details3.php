...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove_ad'])) {
    $sql_remove = 'UPDATE save_ads SET date_remove = NOW() WHERE id_user = ? AND id_ads = ? AND date_remove IS NULL';
    $query_remove = $pdo->prepare($sql_remove);
    $query_remove->execute([$id_user, $ad_id]);
    header("Location: saved_ads.php?ad_id=$ad_id");
    exit();
}
        ...
        <div class="book-details">
            <div class="book-info">
                ...
                <div class="button-container">
                    <a href="saved_ads.php" class="back-button">Назад до оголошень</a>
                    <form action="" method="post">
                        <input type="hidden" name="remove_ad" value="1">
                        <button type="submit" class="deactivate-button">Видалити з вибраного</button>
                    </form>
            ...
            <?php if (!empty($ad['cover'])): ?>
                <img src="http://localhost/bookcrossing/lib/<?php echo htmlspecialchars($ad['cover']); ?>"
                    alt="Обкладинка книги" class="book-cover-details">
            <?php else: ?>
        ...
        <div class="comments">
            ...
            <?php if ($comments): ?>
                <?php foreach ($comments as $comment): ?>
                    <div class="comment">
                        <p><strong><?php echo htmlspecialchars($comment['login']); ?>:</strong>
